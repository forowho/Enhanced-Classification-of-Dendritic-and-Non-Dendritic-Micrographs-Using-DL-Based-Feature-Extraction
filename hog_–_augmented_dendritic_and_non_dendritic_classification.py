# -*- coding: utf-8 -*-
"""HOG – Augmented Dendritic and non_Dendritic Classification.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IPDGvlfEZtB8_Uc-ARakyItIgXmuT4nW
"""
...
def split_and_move_files(src_dir, train_dir, test_dir, train_ratio=0.8, random_state=42):
    ...
    for file_name in train_files:
        shutil.move(os.path.join(src_dir, file_name), train_dir)

    # Move files to test directory
    for file_name in test_files:
        shutil.move(os.path.join(src_dir, file_name), test_dir)
...
train_dendritic_folder = "/content/Augment_Dendrite"
train_non_dendritic_folder = "/content/Augment_Non_Dendrite"
...
def load_dataset_and_extract_features():
    ...
    for img_name in os.listdir(train_dendritic_folder):
        img_path = os.path.join(train_dendritic_folder, img_name)
    ...
    for img_name in os.listdir(train_non_dendritic_folder):
        img_path = os.path.join(train_non_dendritic_folder, img_name)
        image = cv2.imread(img_path)
        if image is not None:
            features = extract_hog_features(image)
            data.append(features)
...
X_train, y_train = load_dataset_and_extract_features()

X_test = test_dendrite_dir, test_non_dendrite_dir
y_test =
